...
/**
 * @summary Allocates an input stream for a specific channel id
 *
 ...
 * @returns {MumbleInputStream} Input stream
 */
MumbleClient.prototype.inputStreamForChannel = function( channelId, options ) {
    var self = this;
    ...
    // Create the targets parameter based on the channel_id type.
    var targets = [];
    if( typeof channelId !== 'number' ) {
        if( typeof channelId === 'object' ) {
            channelId = channelId.id || channelId.channel_id;
        }
        channelId = Number(channelId) || parseInt(channelId, 10);
    }
    targets.push( { channel_id: channelId, children: !!options.children, links: true } );
