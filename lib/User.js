 ...
 */
User.prototype.inputStream = function(options) {
    return this.client.inputStreamForUser( this.session, options || {} );
};

/**
 * @summary Returns an input stream for sending audio to the channel (with children).
 *
 * @param {Integer} [channelId]
 * @param {Object} [options]
 * @returns {MumbleInputStream} Input stream.
 */
User.prototype.inputChannelStream = function(channelId, options) {
    if (!channelId || !this.client.channelById(channelId)) {
        channelId = this.channel.id;
    }
    return this.client.inputStreamForChannel(channelId || this.channel.id, options || {} );
};
